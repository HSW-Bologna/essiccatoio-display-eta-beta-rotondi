...
void parmac_init(mut_model_t *model, int reset) {
    ...
    ps[i++] = PARAMETER(&p->logo,                                     0,                              NUM_LOGOS - 1,              0,                              FOPT(PARS_DESCRIPTIONS_LOGO, pars_loghi),                                      USER_BITS);
    ...
    ps[i++] = PARAMETER(&p->residual_humidity_check,                  0,                              1,                          0,                              FOPT(PARS_DESCRIPTIONS_CONTROLLO_UMIDITA_RESIDUA, pars_nosi),                    USER_BITS);
    ...
    ps[i++] = PARAMETER(&p->reset_page_time,                          10,                             250,                        30,                             FTIME(PARS_DESCRIPTIONS_TEMPO_DI_RITORNO_PAGINA_INIZIALE),                          USER_BITS);
    ...
    ps[i++] = PARAMETER(&p->max_output_temperature,                   1,                              85,                         85,                             FFINT(PARS_DESCRIPTIONS_TEMPERATURA_MASSIMA_IN_USCITA, fmt_c),                USER_BITS);
    ps[i++] = PARAMETER(&p->safety_output_temperature,                1,                              90,                         90,                             FFINT(PARS_DESCRIPTIONS_TEMPERATURA_DI_SICUREZZA_IN_USCITA, fmt_c),                USER_BITS);
    ps[i++] = PARAMETER(&p->air_flow_alarm_time,                      1,                              250,                        10,                             FTIME(PARS_DESCRIPTIONS_TEMPO_ALLARME_FLUSSO_ARIA),                               USER_BITS);
    ...
    ps[i++] = PARAMETER(&p->tempo_allarme_temperatura,                60,                             99*60+59,                   45*60,                          FTIME(PARS_DESCRIPTIONS_TEMPO_ALLARME_TEMPERATURA),                               USER_BITS);
    ps[i++] = PARAMETER(&p->allarme_inverter_off_on,                  0,                              1,                          1,                              FOPT(PARS_DESCRIPTIONS_DIREZIONE_ALLARME_INVERTER, pars_nc_na),                              USER_BITS);
    ps[i++] = PARAMETER(&p->allarme_filtro_off_on,                    0,                              1,                          1,                              FOPT(PARS_DESCRIPTIONS_DIREZIONE_ALLARME_FILTRO, pars_nc_na),                                USER_BITS);
    ps[i++] = PARAMETER(&p->emergency_alarm_nc_na,                    0,                              1,                          1,                              FOPT(PARS_DESCRIPTIONS_DIREZIONE_ALLARME_EMERGENZA, pars_nc_na),                                USER_BITS);
    ps[i++] = PARAMETER(&p->fan_with_open_porthole_time,              0,                              240,                        3,                              FTIME(PARS_DESCRIPTIONS_TEMPO_VENTILAZIONE_OBLO_APERTO),                          USER_BITS);
